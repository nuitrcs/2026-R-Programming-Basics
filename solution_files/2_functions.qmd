...
replace_e = function(text, replacement = "*") {
  str_replace_all(text, "e", replacement)
}

replace_e("input text")
replace_e("input text", replacement = "000")
```
...
apply_threshold = function(x, threshold) {
  x[x < threshold] = threshold
  return(x)
}

apply_threshold(test_measurements, 0)
```
...
group_mean = function(data, group_var, target_var) {
  data %>%
    group_by({{ group_var }}) %>%
    summarize(group_mean = mean({{ target_var }}, na.rm = TRUE))
}

group_mean(diamonds, cut, price)
```
...
1.  We use `:=` to do the assignment in `mutate()`. Without `:=`, the column would literally be named exactly "na_count\_{{target_col}}" (not substituting in the value for `target_col`)
